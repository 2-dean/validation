...
public class ValidationItemControllerV2 {
    ...
    private final ItemValidator itemValidator; // 컴포넌트 등록하고 빈 주입받기


    @InitBinder
    public void init(WebDataBinder dataBinder) {
        dataBinder.addValidators(itemValidator); //이 컨트롤러가 요청될때 마다 WebDataBinder 가 만들어짐! 검증기 적용가능
    }
    ...
    public String addItemV1(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        // @ModelAttribute Tmaus model.addAttribute
        // BindingResult  :  item에 바인딩된 결과가 담김?, 자동으로 뷰로 넘어감, 있으면
        // ## 꼭 ModelAttribute 객체 다음에 BindingResult가 와야함!! 중요~!
        ...
        if (item.getPrice() != null &&  item.getQuantity() != null ) {
           ...
           if (resultPrice < 10000) {
               // 특정 필드에러가 아닌 경우 new ObjectError
    ...
    public String addItemV3(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        ...
        // bindingResult는 검증해야할 객체인 target을 알고있다
    ...
    public String addItemV6(@Validated @ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes, Model model) {
        // @Validated 를 추가하면 item에 대해 자동으로 검증기 수행됨 addItemV5 와 똑같이 적용
